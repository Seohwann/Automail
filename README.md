...
- 상태 정의는 `agents/state.py`의 `WorkflowState`입니다. 전체 흐름을 한 번에 실행하는 배치 모드는 아래 2-1의 supervisor 멀티 에이전트가 담당합니다.

---

## 2-1. 멀티 에이전트 모드 (Agent 브랜치)

위의 단계별 그래프에 더해, **supervisor 에이전트**가 전체 흐름을 동적으로 지휘하는
멀티 에이전트 모드가 추가되었다 (`agents/supervisor.py`, 대시보드 **4 · 자동 실행** 탭).

```
            ┌→ 검색 에이전트 (ReAct: web_search / open_website 도구를 스스로 선택·반복)
supervisor ─┼→ 작성 에이전트
 (LLM 라우팅)├→ 발송 승인 (interrupt — 사람이 대시보드에서 승인한 업체만 발송)
            ├→ 답장 에이전트 (ReAct: 문의 답변에 필요하면 web_search 로 조사)
            └→ 종료
```

- **검색·답장 에이전트는 ReAct 루프**로 전환: 기존의 고정 4단계 폴백 대신 LLM 이
  도구(`agents/tools.py`)를 보고 업체별 탐색 전략을 스스로 결정한다 (`agents/react.py`).
- **supervisor 가 제어 흐름을 결정**: 검색 실패 시 지시를 바꿔 재시도, 답장이 문의면
  후속 대응 등 조건 분기·재시도를 LLM 이 판단한다.
- **안전장치는 결정적 코드로 유지** (LLM 판단에 맡기지 않음):
  - grounding: 도구가 수집한 이메일 후보와 정확 일치 + 공식 도메인에서 실제 목격된
    경우에만 검증 O (`CandidateStore`)
  - 발송은 LangGraph `interrupt()` 로 그래프를 멈추고 사람 승인을 강제
  - 검색 재시도 상한(업체당 2회)·supervisor 턴 상한(12회)으로 비용/무한 루프 통제
  - supervisor 결정이 무효면 결정적 폴백 규칙(검색→작성→승인→답장 순)으로 진행

기존 1~3탭(고정 그래프)은 그대로 동작하며, 내부적으로 같은 에이전트 코어를 호출한다.
...
├── agents/
│   ├── search_agent.py   # ① 검색 에이전트 (ReAct)
│   ├── writer_agent.py   # ② 작성 에이전트
│   ├── reply_agent.py    # ③ 답장 에이전트 (ReAct)
│   ├── tools.py          # 에이전트 도구 (web_search/open_website) + grounding 후보 저장소
│   ├── react.py          # 경량 ReAct 루프 실행기
│   ├── supervisor.py     # supervisor 멀티 에이전트 그래프 (동적 라우팅 + 발송 승인 interrupt)
│   ├── graph.py          # LangGraph 오케스트레이션 (단계별 고정 그래프, 1~3탭용)
