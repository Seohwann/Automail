...
성균관대학교 총학생회 S'PEAK 대외협력국 업무 과정에서 발생하는 비효율을 줄이기 위해 협찬·부스 입점 제안 요청 메일 발송을 자동으로 처리해주는 MutliAgent 관련 개인 프로젝트입니다.
구글 시트에 업체 목록(업체명·힌트)만 넣으면, **이메일 자동 탐색 → 업체별 맞춤 제안 메일 작성 → 일괄 발송 → 답장 분류·후속 대응**까지 이어지는 협찬/제안 메일 자동화 도구입니다. 
...
- **역할**: 업체명(+ 시트의 업종/키워드 힌트)으로 웹을 검색해 협찬/제휴 문의 **이메일을 찾아내고 신뢰도를 판정**합니다.
- **동작**:
  1. Tavily로 `"{업체명} {힌트} 협찬 제휴 문의 이메일"` 검색
  2. Gemini가 결과에서 이메일 추출 + 업종 일치 여부·신뢰도 판단
  3. **Grounding**: 추출된 이메일이 검색 결과 원문에 실제로 등장하는지 확인 → 없는 주소(환각)는 폐기
  4. **등급 부여**: `HIGH`(검증 O) / `REVIEW`(검토 필요) / `NONE`(미발견)
